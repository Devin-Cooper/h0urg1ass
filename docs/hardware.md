...
| `main/main.cpp` prologue | The `set_sys_clock_khz()` → `clock_configure(clk_peri, …CLKSRC_PLL_SYS…)` sequence (lines 177–191). **Do not carry over the reference project's `vreg_set_voltage(VREG_VOLTAGE_1_20)`:** DVDD is specified 1.05 / 1.10 / **1.16 V max** [RP2350 datasheet Table 1441], so 1.20 V is out of spec, and the core regulator is a buck — raising it costs roughly +2.8 mA at the battery rail for nothing. `SYS_CLK_VREG_VOLTAGE_AUTO_ADJUST` defaults to 0, so the SDK does not raise it behind your back, the 5-second countdown idle (198–201), `freeHeap()` via `extern "C" char __StackLimit / __bss_end__` (36–37, 59–63), and the whole measurement harness |
